...
const tracingHeaders = ['x-request-id','x-b3-traceid', 'x-b3-spanid', 'x-b3-parentspanid',
...
router.routes = (store) => {
  router.get('/bonjour', (req, resp) => {
      console.log('Request Headers Received:'+JSON.stringify(req.headers)); // log to see what headers we get
  ...
  router.get('/bonjour-chaining', (req, resp) => {
    // START -- Propagate tracing headers
     tracingHeaders.forEach(function(tHeaderName){
         var tHeaderValue = req.header(tHeaderName);
         console.log(`Adding Tracing Header:${tHeaderName} having value ${tHeaderValue}`);
         req.headers[tHeaderName] = tHeaderValue;
      });
    // END -- Propagate tracing headers
    circuit.fire({endpoint: `http://istio-ingress/api/${nextService}-chaining`}).then((response) => {
      resp.set('Access-Control-Allow-Origin', '*');
      resp.type('application/json').send(response.body);
    }).catch((e) => resp.send(e)) }
